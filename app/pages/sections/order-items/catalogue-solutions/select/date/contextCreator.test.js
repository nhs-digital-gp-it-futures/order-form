...
const data = { 'plannedDeliveryDate-day': '10', 'plannedDeliveryDate-month': '10', 'plannedDeliveryDate-year': '2020' };
...
describe('delivery-date contextCreator', () => {
  describe('getContext', () => {
    ...
    it('should populate the delivery date with the commencement date passed in', () => {
      ...
      const { plannedDeliveryDate } = context.questions;
      expect(plannedDeliveryDate.id).toEqual('plannedDeliveryDate');
      expect(plannedDeliveryDate.type).toEqual('date');
      expect(plannedDeliveryDate.mainAdvice).toEqual('Planned delivery date');
      expect(plannedDeliveryDate.additionalAdvice).toEqual('For example 14 01 2020');
      expect(plannedDeliveryDate.expandableSection.dataTestId).toEqual('view-section-planned-delivery-date-id');
      expect(plannedDeliveryDate.expandableSection.title).toEqual('What date should I enter?');
      expect(plannedDeliveryDate.expandableSection.innerComponent).toEqual('Enter the planned delivery date you\'ve agreed with the supplier. This is the day that each of the listed Service Recipients will start using this Catalogue Solution.<br><br>We\'ve included the commencement date for this Call-off Agreement, but you can change it if you’ve agreed a different delivery date.');
      expect(plannedDeliveryDate.data).toEqual({ day: '10', month: '10', year: '2020' });
    });
  });
  ...
  describe('getErrorContext', () => {
    it('should return error for plannedDeliveryDate', () => {
      const expectedContext = {
        errors: [
          { href: '#plannedDeliveryDate', text: manifest.errorMessages.PlannedDeliveryDateRequired },
        ],
        questions: {
          ...manifest.questions,
          plannedDeliveryDate: {
            ...manifest.questions.plannedDeliveryDate,
            error: {
              message: manifest.errorMessages.PlannedDeliveryDateRequired,
