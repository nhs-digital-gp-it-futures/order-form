...
const setup = {
  template: {
    path: 'pages/sections/order-items/associated-services/dashboard/template.njk',
  },
};

describe('Associated-services - Dashboard page', () => {
  ...
  it('should render the associated-services page title', componentTester(setup, (harness) => {
    const context = {
      title: 'Associated Services for order-1',
    };

    harness.request(context, ($) => {
      const title = $('h1[data-test-id="associated-services-page-title"]');
      expect(title.length).toEqual(1);
      expect(title.text().trim()).toEqual(context.title);
    });
  }));

  it('should render the associated-services page description', componentTester(setup, (harness) => {
    const context = {
      description: manifest.description,
    };

    harness.request(context, ($) => {
      const description = $('h2[data-test-id="associated-services-page-description"]');
  ...
  it('should render the additional-services page inset advice', componentTester(setup, (harness) => {
    ...
    harness.request(context, ($) => {
      const insetAdvice = $('[data-test-id="associated-services-page-insetAdvice"]');
  ...
  it('should render the "Add Associated Services" button', componentTester(setup, (harness) => {
    const context = {
      addOrderItemButtonText: 'Add Associated Services',
      addOrderItemButtonHref: '/organistaions/order-1/associated-services/select',
    };
  ...
  it('should render no associated services text when the items provided is an empty array', componentTester(setup, (harness) => {
    const context = {
  ...
  describe('Added Order Items table', () => {
    ...
    it('should render the table headings', componentTester(setup, (harness) => {
      harness.request(context, ($) => {
        ...
        expect(table.find('[data-test-id="column-heading-0"]').text().trim()).toEqual('Associated Service');
    ...
    it('should render the data', componentTester(setup, (harness) => {
      harness.request(context, ($) => {
        ...
        expect(row1catalogueItemName.text().trim()).toEqual('Associated Service One');
        ...
        expect(row2catalogueItemName.text().trim()).toEqual('Associated Service Two');
