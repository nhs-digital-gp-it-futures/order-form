...
fixture('Solution - Dashboard page - without saved data')
...
test('should render the added solutions table with the column headings', async (t) => {
  await pageSetup();
...
test('should render the added solutions items in the table', async (t) => {
  await pageSetup();
  await t.navigateTo(pageUrl);
  ...
  await t
    ...
    .expect(row1SolutionName.getAttribute('href')).eql(`${baseUrl}/organisation/order-1/solution/orderItem1`)
    ...
    .expect(row2SolutionName.getAttribute('href')).eql(`${baseUrl}/organisation/order-1/solution/orderItem2`)
