...
const pageUrl = 'http://localhost:1234/order/organisation/order-id/solution/existing-order-id';
...
const mocks = () => {
  nock(orderApiUrl)
    .get('/api/v1/orders/order-id/sections/solution/existing-order-id')
    .reply(200, orderItem);
};
...
test('should navigate to /organisation/order-id/solution/select/solution/price/recipient when click on backlink when not a new order item', async (t) => {
  await pageSetup();
  await t.navigateTo(pageUrl);
...
test('should navigate to catalogue solution dashboard page if save button is clicked and data is valid', async (t) => {
  nock(orderApiUrl)
    .put('/api/v1/orders/order-id/sections/solution/existing-order-id')
    .reply(200, {});
